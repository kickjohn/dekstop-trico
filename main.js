...
// Lottie JSON URLs (dalam kode ini menggunakan placeholder URL)
const lottieFiles = {
    neutral: 'normal.json',
    sad: 'sad.json',
    listening: 'berbicara.json',
    blink: 'kedipOk.json',
    happy: 'terimaKasih.json',
    tired: 'tidur.json'
};

// Inisialisasi komponen saat DOM telah dimuat
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup tampilan awal dan interaksi
    setupEventListeners();
    
    // Mulai interval kedipan (setiap 2 menit)
    startBlinkInterval();
...
// Fungsi untuk menampilkan dialog setting API key
...
function addLoadingIndicator() {
    ...
    typingSpan.textContent = 'Trico sedang mengetik';
...
function enableDemoMode() {
    ...
    // Override fungsi processMessage untuk mode demo
    ...
    processMessage = function(message) {
        ...
        setTimeout(() => {
            ...
            // Generate respons demo sesuai pesan
...
// Fungsi untuk mendapatkan respons demo
function getDemoResponse(message) {
    message = message.toLowerCase();
    
    // Respons demo berdasarkan kata kunci
...
// Tambahkan opsi mode demo dalam dialog API key
...
// Panggil fungsi untuk memperbarui dialog API key
updateApiKeyDialog();

// Cek apakah perlu memulai mode demo
if (!apiKey) {
    // Jika tidak ada API key, tampilkan dialog API key
    // yang sudah diupdate dengan opsi mode demo
    showApiKeyDialog();
}
